...
QFuture<void> processScreenshot(
	...
	const cv::Rect&  rect,
	const OCREngine& ocr_engine,
	TooltipWnd*      tooltip_window
);
...
int main(int argc, char* argv[]) {
	...
	QObject::connect(
		...
		&app,
		[tooltip_window, &engine, screenshot_window](const cv::Mat& screenshot_mat, const cv::Rect& rect) {
			screenshot_window->hide();
...
QFuture<void> processScreenshot(
	...
	return QtConcurrent::run(
		...
		[&rect, tooltip_window](const std::vector<OCRResult>& res) {
